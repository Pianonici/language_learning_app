...
class UserProgress(object):
    def __init__(self, user_name, german_to_english_dictionary):
        self._filepath = Path(f'user_progress_{user_name}.txt') #we need this for the filepath because we are creating a file
        self._word_to_score = {}
            ...
            for line in lines:
                ...
                if line.strip() != "":
                    ...
                    if score in self._score_to_words:
                        self._score_to_words[score].append(combined_word) #appending the tuple to the list of words with same score
                    else:
                        self._score_to_words[score] = [combined_word] #if it's the first score with that score, map the score to a list of tuples with the given tuple as the first element of that list 
            file.close()
    ...
    def add_point(self, german_word, english_word):
        ...
        if combined_word in self._word_to_score:
            ...
            if current_score +1 in self._score_to_words:
                self._score_to_words[current_score +1].append(combined_word) #append the tuple to the new score list if list already exists 
            else:
                self._score_to_words[current_score +1] = [combined_word] #if new score list does not already exist, create new list
    ...
    def save_progress(self):
        ...
        for combined_word, score in self._word_to_score.items(): #.items() goes through all elements in the dictionary and gives us tuples. Each element looks like: ((Abend, evening), -1)
